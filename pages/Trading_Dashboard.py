...
# Add paths
sys.path.insert(0, os.path.join(os.path.dirname(os.path.dirname(__file__)), 'TradingAgents'))
sys.path.insert(0, os.path.dirname(os.path.dirname(__file__)))

from utils.styles import get_tradeguide_styles, get_sidebar_html, get_page_header, get_footer_html

# Set page config
st.set_page_config(
    page_title="Trading Dashboard - TradeGuide AI",
    page_icon="📊",
    layout="wide"
)
...
# Page Header
st.markdown(get_page_header("📊 Trading Dashboard", "AI-powered analysis and trading recommendations"), unsafe_allow_html=True)
...
with col2:
    ...
    # Check if Alpha Vantage is selected to load tickers from CSV
    if data_vendor == "Alpha Vantage":
        # Load Alpha Vantage tickers from CSV
        try:
            alpha_vantage_csv_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'alpha_vantage_combined_tickers.csv')
            av_df = pd.read_csv(alpha_vantage_csv_path)
            
            # Group tickers by type
            ALPHA_VANTAGE_TICKERS = {}
            for ticker_type in av_df['type'].dropna().unique():
                type_df = av_df[av_df['type'] == ticker_type]
                # Format: (name, symbol)
                ALPHA_VANTAGE_TICKERS[ticker_type] = [(row['name'], row['symbol']) for _, row in type_df.iterrows()]
            
            # Add category filter
            available_categories = list(ALPHA_VANTAGE_TICKERS.keys())
            selected_category = st.selectbox(
                "Select Category",
                options=available_categories,
                help="Choose a category to filter tickers"
            )
            
            # Flatten the ticker list for selectbox with display names (filtered by category)
            all_tickers_display = []
            ticker_map = {}  # Map display name to ticker symbol
            
            if selected_category in ALPHA_VANTAGE_TICKERS:
                for name, symbol in ALPHA_VANTAGE_TICKERS[selected_category]:
            ...
            ticker_option = st.selectbox(
                f"Select Ticker ({selected_category})",
                options=["Custom"] + all_tickers_display,
                index=1 if len(all_tickers_display) > 0 else 0,
                help=f"Select from {selected_category} tickers or choose 'Custom' to enter your own"
            )
            
            # If custom selected, show text input
            if ticker_option == "Custom":
                ticker = st.text_input(
                    "Enter Custom Ticker (Alpha Vantage Format)",
                    value="AAPL",
                    help="Enter ticker symbol in Alpha Vantage format (e.g., AAPL, RELIANCE.BSE, BTC, USD/EUR)"
                ).upper()
                # Display custom ticker label
                st.caption("📊 Category: Custom Ticker")
            else:
            ...
            st.error(f"Error loading Alpha Vantage tickers from CSV: {str(e)}")
            ticker = st.text_input(
                "Enter Ticker Symbol",
                value="AAPL",
        ...
        # Yahoo Finance tickers - Load from CSV
        try:
            yahoo_finance_csv_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'yahoo_finance_combined_tickers.csv')
            yf_df = pd.read_csv(yahoo_finance_csv_path)
            
            # Group tickers by type
            YAHOO_FINANCE_TICKERS = {}
            for ticker_type in yf_df['type'].dropna().unique():
                type_df = yf_df[yf_df['type'] == ticker_type]
                # Format: (name, symbol)
                YAHOO_FINANCE_TICKERS[ticker_type] = [(row['name'], row['symbol']) for _, row in type_df.iterrows()]
            
            # Add category filter
            available_categories = list(YAHOO_FINANCE_TICKERS.keys())
            selected_category = st.selectbox(
                "Select Category",
                options=available_categories,
                help="Choose a category to filter tickers"
            ...
            if selected_category in YAHOO_FINANCE_TICKERS:
                for name, symbol in YAHOO_FINANCE_TICKERS[selected_category]:
                    display_name = f"{name} - {symbol}"
                    all_tickers_display.append(display_name)
                    ticker_map[display_name] = symbol
            ...
            if ticker_option == "Custom":
                ticker = st.text_input(
                    "Enter Custom Ticker (Yahoo Finance Format)",
                    value="AAPL",
                    help="Enter ticker symbol in Yahoo Finance format (e.g., AAPL, RELIANCE.NS, BTC-USD, EURUSD=X)"
                ).upper()
                # Display custom ticker label
                st.caption("📊 Category: Custom Ticker")
            else:
            ...
            st.error(f"Error loading Yahoo Finance tickers from CSV: {str(e)}")
            ticker = st.text_input(
                "Enter Ticker Symbol",
                value="AAPL",
                help="Enter ticker symbol"
